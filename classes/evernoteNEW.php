...
require_once(__DIR__."/../lib/transport/TTransport.php");
require_once(__DIR__."/../lib/transport/THttpClient.php");
require_once(__DIR__."/../lib/protocol/TProtocol.php");
require_once(__DIR__."/../lib/protocol/TBinaryProtocol.php");
require_once(__DIR__."/../lib/packages/Types/Types_types.php");
require_once(__DIR__."/../lib/packages/UserStore/UserStore.php");
require_once(__DIR__."/../lib/packages/NoteStore/NoteStore.php");
...
class Evernote
{
    protected $access_token, $shard_id, $user_id, $config, $tokens, $noteserver;
    
    protected $evernote_server = 'https://sandbox.evernote.com/oauth';
    
    
    public static function _init()
    {
        $config = \Fuel\Core\Config::load('evernote', true);
        ...
        $this->evernote_server = $config[$config['active']]['evernote_server'];
        $this->noteserver = new StdClass();
        $this->noteserver->host = $config[$config['active']]['noteserver_host'];
        $this->noteserver->port = $config[$config['active']]['noteserver_port'];
        $this->noteserver->protocol = $config[$config['active']]['noteserver_protocol'];        
    ...
    public static function getTemporaryCredentials()
    {
        try
        {
            $request_token_url = $this->evernote_server.'/oauth';
